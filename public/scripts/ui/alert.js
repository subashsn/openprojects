(function ($) {
	...
	$('.demo4').on('click', function () {
		swal('And done!', 'You will hear from your team admin soon!', 'success');
	});
	...
	$('.demo6').on('click', function () {
		swal({
			title: 'Are you sure?',
			text: 'You will not be able to recover this idea.',
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d43d52',
			confirmButtonText: 'Yes, Delete It!',
			...
			if (isConfirm) {
				...
				swal('Cancelled', 'Your idea is safe :)', 'error');
			}
		});
	});
